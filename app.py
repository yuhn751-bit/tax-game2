...
LOGIC_CARD_DB = {
    "c_tier_01": LogicCard(
        ...
        cost=0, base_damage=4, tax_type=['부가세', '법인세'], # (3 -> 4)
        text="자료 대사의 기본을 익혔다."
    ),
    "c_tier_02": LogicCard(
    ...
    "s_tier_02": LogicCard( 
        ...
        cost=4, base_damage=100, tax_type=['법인세'], # (80 -> 100)
    ...
    Company(
        ...
        tactics=[
            EvasionTactic("사주 개인적 사용", 
                          "대표이사가 법인 명의의 롤렉스 시계를 구매하고, 주말 골프 비용을 법인카드로 결제하는 등 개인적인 용도로 자금을 유용함.", # (설명 수정)
                          30, 0.3, tax_type='법인세', method_type='고의적 누락'),
            EvasionTactic("증빙 미비 경비", 
                          "실제 거래 없이 서류상으로만 거래처에 물품을 제공한 것처럼 꾸미고, 관련 증빙(세금계산서, 입금표)을 제시하지 못함.", # (설명 수정)
                          20, 0.5, tax_type='법인세', method_type='단순 오류')
        ],
        defense_actions=[
            "담당 세무사가 시간을 끌고 있습니다.",
    ...
    Company(
        ...
        tactics=[
            EvasionTactic("과면세 오류", 
                          "과세 대상인 '소프트웨어 유지보수' 용역을 면세 대상인 '소프트웨어 개발' 용역으로 위장 신고하여, 부가가치세를 고의로 누락함.", # (설명 수정)
                          80, 0.2, tax_type='부가세', method_type='단순 오류'),
            EvasionTactic("관계사 부당 지원", 
                          "대표이사 아들이 소유한 페이퍼컴퍼니에 '경영 자문' 명목으로, 시가보다 현저히 높은 용역비를 매월 지급하여 부당하게 이익을 이전시킴.", # (설명 수정)
                          120, 0.1, tax_type='법인세', method_type='자본 거래')
        ],
        defense_actions=[
    ...
    Company(
        ...
        tactics=[
            EvasionTactic("허위 세금계산서 발행", 
                          "실제 석유 거래 없이 허위 세금계산서(가짜 석유) 수억 원어치를 발행/수취하여, 매입세액을 부당하게 공제받음.", # (설명 수정)
                          200, 0.3, tax_type='부가세', method_type='고의적 누락'),
            EvasionTactic("가공 경비 계상", 
                          "근무하지 않는 친인척을 직원으로 등록해 허위 인건비를 계상하고, 가공의 복리후생비를 지출하여 법인 소득을 조직적으로 축소함.", # (설명 수정)
                          100, 0.5, tax_type='법인세', method_type='고의적 누락')
        ],
        defense_actions=[
            "대표이사가 잠적했습니다.",
    ...
    Company(
        ...
        tactics=[
            EvasionTactic("일감 몰아주기", 
                          "총수 일가가 지분 100%를 소유한 'A사'에 그룹 내부 시스템(SI) 용역을 고가로 발주하여 막대한 이익을 몰아줌.", # (설명 수정)
                          500, 0.1, tax_type='법인세', method_type='자본 거래'),
            EvasionTactic("가공 세금계산서 수취", 
                          "실제 거래가 없는 유령 회사로부터 수백억 원대의 가짜 세금계산서를 받아 비용을 부풀리고 부가세를 환급받음.", # (설명 수정)
                          300, 0.0, tax_type='부가세', method_type='고의적 누락'),
            EvasionTactic("불공정 합병", 
                          "총수 일가에 유리하도록 계열사 간 합병 비율을 의도적으로 조작하여, 편법으로 경영권 승계 및 이익을 증여함.", # (설명 수정)
                          200, 0.0, tax_type='법인세', method_type='자본 거래')
        ],
        defense_actions=[
    ...
    Company(
        ...
        tactics=[
            EvasionTactic("이전가격(TP) 조작", 
                          "아일랜드 등 저세율 국가의 본사에 과도한 '경영자문료'와 '소프트웨어 라이선스비'를 지급하여 국내 이익을 의도적으로 축소함.", # (설명 수정)
                          500, 0.1, tax_type='법인세', method_type='자본 거래'),
            EvasionTactic("고정사업장 미신고", 
                          "국내에 서버 등 실질적인 사업장을 운영함에도, 이를 '연락 사무소'로 위장 신고하여 법인세를 회피함.", # (설명 수정)
                          300, 0.2, tax_type='법인세', method_type='고의적 누락')
        ],
        defense_actions=[
    ...
    Company(
        ...
        tactics=[
            EvasionTactic("역외탈세 (SPC)", 
                          "BVI(영국령 버진아일랜드)에 설립한 다수의 페이퍼컴퍼니(SPC)를 이용해 선박 운용 리스료 등 국내 소득을 은닉함.", # (설명 수정)
                          1000, 0.1, tax_type='법인세', method_type='자본 거래'),
            EvasionTactic("선박 취득가액 조작", 
                          "선박을 해외 SPC에 저가로 양도한 뒤, 해당 SPC가 다시 고가로 매각하는 방식으로 양도 차익을 은닉함.", # (설명 수정)
                          500, 0.2, tax_type='법인세', method_type='고의적 누락')
        ],
        defense_actions=[
...
def draw_cards(num_to_draw):
    ...
    # start_player_turn 함수에서만 호출하도록 변경됨.
...
def calculate_card_cost(card):
    ...
    if "백용후" in [m.name for m in st.session_state.player_team] and ('데이터' in card.name or '분석' in card.name): 
...
def execute_attack(card_index, tactic_index):
    ...
    # [신규] 패널티 로직
    is_match = False
    if '공통' in card.tax_type: is_match = True
    elif tactic.tax_type in card.tax_type: is_match = True
        
    if not is_match:
        log_message(f"🚨 [조사 실패] '{card.name}' 카드는 '{tactic.tax_type}' 혐의에 사용할 수 없습니다!", "error")
        log_message("⚠️ 잘못된 접근으로 팀 체력이 10 감소합니다!", "warning")
        ...
        st.session_state.player_discard.append(st.session_state.player_hand.pop(card_index))
        st.session_state.selected_card_index = None
        check_battle_end() # 패널티로 인한 사망 체크
        st.rerun()
        return # 공격 종료

    # --- (이하 로직은 '일치'했을 경우 정상 진행) ---
    cost_to_pay = calculate_card_cost(card)
...
def show_main_menu():
    ...
    with st.expander("📖 게임 방법 (필독!)", expanded=True):
        ...
        **3. ⚠️ [신규] 핵심 규칙: 세목 불일치 '패널티'**
        - 모든 카드와 혐의에는 **'법인세'** 또는 **'부가세'** 태그가 붙어있습니다.
        - **'법인세' 카드로 '부가세' 혐의를 공격**할 수 있지만...
        - 이 경우, 공격은 실패하고 **'팀 체력'이 10 감소하는 패널티**를 받습니다!
        - `공통` 또는 `법인세, 부가세` 태그가 붙은 카드는 패널티 없이 모두 공격 가능합니다.
        - 공격 버튼에 `⚠️ (세목 불일치)` 경고가 뜨면 주의하세요!
        ...
        **5. 🖱️ 조작법**
        ...
        - **[전투]** 화면 (플레이어 턴):
            ...
            2. 가운데 **'탈루 혐의 목록'** 에서 공격할 혐의의 **[🎯 (혐의 이름) 공격]** 버튼을 누릅니다.
    ...
    with col_mid:
        ...
        for i, tactic in enumerate(company.tactics):
            ...
            with st.container(border=True):
                ...
                # [수정] 길어진 설명을 위해 | {clarity_text} 제거
                st.caption(f"_{tactic.description}_") 
                st.caption(clarity_text) # 명확도 별도 라인
                ...
                if is_card_selected and not tactic_cleared:
                    ...
                    is_match = False
                    if '공통' in selected_card.tax_type: is_match = True
                    elif tactic.tax_type in selected_card.tax_type: is_match = True
                    
                    button_label = f"🎯 **{tactic.name}** 공격"
                    button_type = "primary"
                    if not is_match:
                        button_label = f"⚠️ (세목 불일치) {tactic.name} 공격"
                        button_type = "secondary" # 경고를 위해 버튼 스타일 변경
