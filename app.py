...
LOGIC_CARD_DB = {
    ...
    # (보호막 카드 제거됨)
    # "def_01": LogicCard(name="과세자료 검토", ...),
    # "def_02": LogicCard(name="법률 자문", ...),
...
ARTIFACT_DB = {
    ...
    # "vest": Artifact(name="🛡️ 방탄 조끼", description="전투 시작 시 보호막 +30.", effect={"type": "on_battle_start", "value": 30, "subtype": "shield"}), # 제거됨
...
COMPANY_DB = [
    Company(
        ...
        revenue=8000, operating_income=800, tax_target=5, team_hp_damage=(5, 10), # 매출/이익 상향
        description="중소 유통업체. 사장 SNS는 슈퍼카와 명품 사진 가득.",
        real_case_desc="[교육] 소규모 법인은 대표가 법인 자금을 개인 돈처럼 쓰는 경우가 빈번합니다...", # (설명 동일)
        tactics=[
    ...
    Company(
        ...
        revenue=150000, operating_income=15000, tax_target=80, team_hp_damage=(15, 30), # 스탯 상향
        description="급성장 게임/IT 기업. 복잡한 지배구조와 관계사 거래.",
        real_case_desc="[교육] 2001.7.1. 이후 SW 개발·유지보수 용역은 원칙적으로 과세(10%)입니다...", # (설명 동일)
        tactics=[
        ...
        defense_actions=["회계법인이 '정상 거래' 주장.", "자료가 '서버 오류'로 삭제 주장.", "실무자가 '모른다'며 비협조."] # 효과 텍스트 제거
    ),
    Company(
        name="(주)한늠석유 (자료상)", size="중견기업",
        revenue=50000, operating_income=-1000, tax_target=100, team_hp_damage=(20, 35), # 목표/데미지 상향
        description="전형적인 '자료상'. 가짜 석유 유통, 허위 세금계산서 발행.",
        real_case_desc="[교육] '자료상'은 폭탄업체, 중간 유통 등 여러 단계를 거쳐 허위 세금계산서를 유통시킵니다...", # (설명 동일)
        tactics=[
        ...
        defense_actions=["대표 해외 도피.", "사무실 텅 빔 (페이퍼컴퍼니).", "대포폰/대포통장 외 단서 없음."] # 효과 텍스트 제거
    ),
     Company(
        name="㈜삼숭물산 (Samsoong)", size="대기업",
        revenue=50_000_000, operating_income=2_000_000, tax_target=1000, team_hp_damage=(20, 40), # 유지
        description="대한민국 최고 대기업. 복잡한 순환출자, 경영권 승계 이슈.",
        real_case_desc="[교육] 대기업 경영권 승계 시 '일감 몰아주기'는 단골 탈루 유형입니다...", # (설명 동일)
        tactics=[
        ...
        defense_actions=["최고 로펌 '김&장' 대응팀 꾸림.", "로펌 '정상 경영 활동' 의견서 제출.", "언론에 '과도한 세무조사' 여론전 (팀 체력 -5).", "정치권 통해 조사 중단 압력."] # 효과 텍스트 제거
    ),
    Company(
        name="구갈 코리아(유) (Googal)", size="외국계",
        revenue=2_000_000, operating_income=300_000, tax_target=800, team_hp_damage=(18, 35), # 데미지 상향
        description="다국적 IT 기업 한국 지사. '이전가격(TP)' 조작 통한 소득 해외 이전 혐의.",
        real_case_desc="[교육] 다국적 IT 기업은 조세 조약 및 세법 허점을 이용한 공격적 조세회피(ATP) 전략을 사용합니다...", # (설명 동일)
        tactics=[
        ...
        defense_actions=["미 본사 '영업 비밀' 이유로 자료 제출 거부.", "조세 조약 근거 상호 합의(MAP) 신청 압박.", "자료 영어로만 제출, 번역 지연.", "집중력 감소 유도."] # 효과 텍스트 제거
    ),
    Company(
        name="(주)씨엔해운 (C&)", size="대기업",
        revenue=10_000_000, operating_income=500_000, tax_target=1500, team_hp_damage=(25, 45), # 유지
        description="'선백왕' 운영 해운사. 조세피난처 페이퍼컴퍼니 이용 탈루 혐의.",
        real_case_desc="[교육] 선박 등 고가 자산 산업은 조세피난처(Tax Haven) SPC를 이용한 역외탈세가 빈번합니다...", # (설명 동일)
        tactics=[
        ...
        defense_actions=["해외 법인 대표 연락 두절.", "이면 계약서 존재 첩보.", "국내 법무팀 '해외 법률 검토 필요' 대응 지연.", "조사 방해 시도 (팀 체력 -10)."] # 효과 텍스트 제거
    ),
]

# --- 3. 게임 상태 초기화 및 관리 ---
# --- [수정됨] initialize_game (보호막 제거, 시작 덱 변경) ---
def initialize_game(chosen_lead: TaxManCard, chosen_artifact: Artifact):
    seed = st.session_state.get('seed', 0)
    if seed != 0:
        random.seed(seed); st.toast(f"ℹ️ RNG 시드 {seed}로 고정됨.")
    else:
        random.seed()
    ...
    # (수정) 시작 덱: 보호막 카드 대신 '법령 재검토'(드로우) 2장 추가
    start_deck = [
        LOGIC_CARD_DB["basic_01"], # 기본 경비 (비용) x 3
        LOGIC_CARD_DB["basic_01"],
        LOGIC_CARD_DB["basic_01"],
        LOGIC_CARD_DB["basic_02"], # 단순 경비 (비용) x 2
        LOGIC_CARD_DB["basic_02"],
        LOGIC_CARD_DB["b_tier_04"], # 세금계산서 (수익/비용) x 2
        LOGIC_CARD_DB["b_tier_04"],
        LOGIC_CARD_DB["c_tier_02"], # 법령 재검토 (드로우) x 2 <-- 보호막 대체
        LOGIC_CARD_DB["c_tier_02"],
        LOGIC_CARD_DB["c_tier_01"], # 단순 자료 대사 (공격/공통) x 5
        LOGIC_CARD_DB["c_tier_01"],
        LOGIC_CARD_DB["c_tier_01"],
        LOGIC_CARD_DB["c_tier_01"],
        LOGIC_CARD_DB["c_tier_01"]
    ] # 총 14장
    ...
    # (수정) 보호막 아티팩트 제외하고 초기화
    st.session_state.player_artifacts = []
    if chosen_artifact.name != "🛡️ 방탄 조끼":
         st.session_state.player_artifacts.append(chosen_artifact)
...
def start_player_turn():
    ...
    if "김대지" in [m.name for m in st.session_state.player_team] and st.session_state.team_stats["data"] >= 50:
        if 'kim_dj_effect_used' not in st.session_state or st.session_state.kim_dj_effect_used == False:
            new_card = copy.deepcopy(LOGIC_CARD_DB["b_tier_01"]) 
    ...
    if "전진" in [m.name for m in st.session_state.player_team]:
        st.session_state.cost_reduction_active = True 
...
def execute_attack(card_index, tactic_index):
    ...
    # --- 페널티 체크 (세목/유형 불일치) ---
    is_tax_match = False
    if TaxType.COMMON in card.tax_type: is_tax_match = True
    elif isinstance(tactic.tax_type, list): is_tax_match = any(tt in card.tax_type for tt in tactic.tax_type)
    else: is_tax_match = tactic.tax_type in card.tax_type
    ...
    if AttackCategory.COMMON in card.attack_category: is_category_match = True
    else: is_category_match = tactic.tactic_category in card.attack_category
    ...
    reference_target = 500 # 기준 목표 세액 (평균 약 500억)
    scaling_factor = (company.tax_target / reference_target)**0.5 # 제곱근 스케일링
    capped_scaling_factor = max(0.5, min(2.0, scaling_factor)) # 0.5배 ~ 2.0배 제한
    scaled_damage = int(base_damage * capped_scaling_factor)
    
    scaling_log = ""
    if capped_scaling_factor != 1.0:
        scaling_log = f" (기업 규모 보정: {base_damage} -> {scaled_damage})"
        
    damage = scaled_damage # 스케일링된 데미지로 시작
    ...
    if "유재준" in [m.name for m in st.session_state.player_team] and tactic.method_type == MethodType.ERROR:
         ...
         if bonus > 0: damage += bonus; log_message(f"✨ [정기 조사 전문] 설득 기반 피해 +{bonus}!", "info")
    if "김태호" in [m.name for m in st.session_state.player_team] and AttackCategory.CAPITAL in card.attack_category:
        ...
        if bonus > 0: damage += bonus; log_message(f"✨ [심층 기획 조사] 증거 기반 피해 +{bonus}!", "info")
    ...
    # --- 로그 출력 (스케일링 정보 포함) ---
    log_prefix = "▶️ [적중]"
    if bonus_multiplier >= 2.0: log_prefix = "💥 [치명타!]"
    elif bonus_multiplier > 1.0: log_prefix = "👍 [효과적!]"
...
def enemy_turn():
    ...
    st.session_state.team_hp -= damage # HP에 바로 데미지
...
# --- [수정됨] start_battle (보호막 아티팩트 로직 제거) ---
def start_battle(company_template):
    ...
    for artifact in st.session_state.player_artifacts:
        ...
        if artifact.effect["type"] == "on_battle_start":
            # if artifact.effect["subtype"] == "shield": ... # 제거됨
            if artifact.effect["subtype"] == "draw":
...
def show_main_menu():
    ...
    if st.button("🚨 조사 시작 (신규 게임)", type="primary", use_container_width=True):
        ...
        # (수정) 보호막 아티팩트 제외하고 샘플링
        artifact_keys = [k for k in ARTIFACT_DB.keys() if ARTIFACT_DB[k].name != "🛡️ 방탄 조끼"]
    ...
    with st.expander("📖 게임 방법 (필독!)", expanded=True):
        ...
        **2. ⚔️ 전투 방식**
        - ❤️ **팀 체력:** 0 되면 패배. 
        - 🧠 **집중력:** 카드 사용 자원 (매우 적음).
        **3. ⚠️ 패널티 시스템 (중요!)** - **1. 세목 불일치:** ... **팀 체력 -10**.
        - **2. 유형 불일치:** ... **팀 체력 -5**.
        - 공격 버튼 `⚠️ (불일치)` 경고 주의! (클릭 불가)
        **4. ✨ 유형 보너스**
        - 혐의에는 `고의적 누락`, `단순 오류`, `자본 거래` 등 ...
        - `현장 압수수색`은 '고의적 누락'에 2배, `판례 제시`는 '단순 오류'에 2배.
        """) # 보호막 설명 제거됨

# --- [수정됨] show_setup_draft_screen (보호막 아티팩트 제거) ---
def show_setup_draft_screen():
    ...
    if 'draft_team_choices' not in st.session_state or 'draft_artifact_choices' not in st.session_state:
        st.error("드래프트 정보가 없습니다...") # 이하 동일
        if st.button("메인 메뉴로"): st.session_state.game_state = "MAIN_MENU"; st.rerun()
        return
    ...
    selected_lead_index = st.radio("리더 후보", options=range(len(team_choices)), format_func=lambda i: f"**{team_choices[i].name} ({team_choices[i].grade}급)** | ...", label_visibility="collapsed") # 이하 동일

    st.markdown("---")
    st.subheader("2. 시작 조사도구를 선택하세요:")
    # (수정) 선택지에 보호막 아티팩트가 없을 것임
    selected_artifact_index = st.radio("조사도구 후보", options=range(len(artifact_choices)), format_func=lambda i: f"**{artifact_choices[i].name}** | {artifact_choices[i].description}", label_visibility="collapsed")
    ...
    if st.button("이 구성으로 조사 시작", type="primary", use_container_width=True):
        ...
        del st.session_state.draft_team_choices; del st.session_state.draft_artifact_choices
        st.rerun()

def show_map_screen():
    if 'current_stage_level' not in st.session_state:
        st.warning("게임 상태가 초기화되지 않았습니다..."); st.session_state.game_state = "MAIN_MENU"; st.rerun(); return
...
# --- [수정됨] show_battle_screen (보호막 표시 제거) ---
def show_battle_screen():
                ...
                with st.container(border=True):
                    ...
                    if is_card_selected and not tactic_cleared:
                        ...
                        if selected_card.special_bonus and selected_card.special_bonus.get('target_method') == tactic.method_type:
                            button_label = f"💥 [특효!] **{tactic.name}** 공격"; button_type = "primary"; help_text = f"클릭하여 공격! ({selected_card.special_bonus.get('bonus_desc')})"
                        else:
                            button_label, button_type = f"🎯 **{tactic.name}** 공격", "primary"; help_text = "클릭하여 이 혐의를 공격합니다."

                        if not is_tax_match:
                            button_label, button_type = f"⚠️ (세목 불일치!) {tactic.name}", "secondary"; help_text = f"세목 불일치! ... (페널티: ❤️-10)"
                        elif not is_category_match:
                            button_label, button_type = f"⚠️ (유형 불일치!) {tactic.name}", "secondary"; help_text = f"유형 불일치! ... (페널티: ❤️-5)"
    ...
    with col_log_action: # 로그, 행동
        ...
        # c2.metric("팀 보호막", f"{st.session_state.get('team_shield', 0)}") # 제거됨
...
# --- [수정됨] show_reward_screen (보호막 카드 제외, 보상 가중치) ---
def show_reward_screen():
    ...
    with tab1:
        ...
        if 'reward_cards' not in st.session_state or not st.session_state.reward_cards:
            ...
            final_reward_options = []
            has_capital_tx = any(t.method_type == MethodType.CAPITAL_TX for t in company.tactics)

            if has_capital_tx:
                capital_cards = [c for c in reward_pool if AttackCategory.CAPITAL in c.attack_category and c not in final_reward_options]
                if capital_cards:
                    chosen_capital_card = random.choice(capital_cards)
                    final_reward_options.append(chosen_capital_card)
                    st.toast("ℹ️ [보상 가중치] '자본' 유형 카드 1장 포함!")

            remaining_pool = [c for c in reward_pool if c not in final_reward_options]
            num_to_add = 3 - len(final_reward_options)
            
            if len(remaining_pool) < num_to_add:
                final_reward_options.extend(random.sample(remaining_pool, len(remaining_pool)))
            else:
                final_reward_options.extend(random.sample(remaining_pool, num_to_add))
            
            while len(final_reward_options) < 3 and len(reward_pool) > 0:
                 card_to_add = random.choice(reward_pool)
                 if card_to_add not in final_reward_options or len(reward_pool) < 3:
                     final_reward_options.append(card_to_add)

            st.session_state.reward_cards = final_reward_options
    ...
    with tab3:
        ...
        # (수정) 제거 대상 카드 이름 변경 (보호막 카드 제외, 단순자료대사 포함)
...
# --- [수정됨] show_reward_remove_screen (제거 대상 카드 수정) ---
def show_reward_remove_screen():
    ...
    if not removable_cards:
        ...
        if st.button("맵으로 돌아가기"): go_to_next_stage()
        return
            ...
            with st.container(border=True):
                ...
                if st.button(f"제거: {card.name}", key=f"remove_{i}", use_container_width=True, type="primary"):
                    try: # 덱에서 제거 시도
                        ...
                        st.toast(f"덱에서 [{name}] 1장 제거!", icon="🗑️"); go_to_next_stage(); return
                        ...
                        try: # 버린 덱에서 제거 시도
                            ...
                            st.toast(f"버린 덱에서 [{name}] 1장 제거!", icon="🗑️"); go_to_next_stage(); return
                        except (StopIteration, ValueError):
...
def show_game_over_screen():
    ...
    st.image("https://images.unsplash.com/photo-1554224155-16954a151120?...", caption="지친 조사관들...", width=400) # URL 동일
    if st.button("다시 도전", type="primary", use_container_width=True):
        st.session_state.game_state = "MAIN_MENU"; st.rerun()

# --- [수정됨] show_player_status_sidebar (보호막 표시 제거) ---
def show_player_status_sidebar():
    with st.sidebar:
        ...
        with st.expander("📊 팀 스탯", expanded=False):
            ...
            st.markdown(f"- 분석력: {stats['analysis']}") # 이하 동일
        ...
        for member in st.session_state.player_team:
             with st.expander(f"**{member.name}** ({member.grade}급)"):
                 st.write(f"HP:{member.hp}/{member.max_hp}, Focus:{member.focus}") # 이하 동일
...
def main():
    ...
    elif st.session_state.game_state == "GAME_SETUP_DRAFT": show_setup_draft_screen()
    ...
    elif st.session_state.game_state == "REWARD_REMOVE": show_reward_remove_screen()
