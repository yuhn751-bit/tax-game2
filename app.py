...
class EvasionTactic:
    def __init__(self, name, description, total_amount, tax_type, method_type, tactic_category): # clarity 제거됨
        self.name = name
        self.description = description
...
# [조사관 DB] (변경 없음)
TAX_MAN_DB = {
    "lim": TaxManCard(name="임향수", grade_num=4, position="팀장", cost=0, hp=150, focus=4, analysis=10, persuasion=10, evidence=10, data=10, description="국세청 최고의 '조사통'.", ability_name="[기획 조사]", ability_desc="매 턴 집중력 +1. 분석/데이터 카드 피해 +10."),
    "han": TaxManCard(name="한중히", grade_num=5, position="팀장", cost=0, hp=100, focus=3, analysis=9, persuasion=6, evidence=8, data=9, description="국제조세 최고 권위자.", ability_name="[역외탈세 추적]", ability_desc="외국계/자본 거래 피해 +30%."),
    "baek": TaxManCard(name="백용호", grade_num=5, position="팀장", cost=0, hp=110, focus=3, analysis=7, persuasion=10, evidence=9, data=7, description="'과학적 세정' 선구자.", ability_name="[TIS 분석]", ability_desc="데이터 카드 비용 -1."),
    "seo": TaxManCard(name="서영택", grade_num=6, position="팀장", cost=0, hp=120, focus=3, analysis=8, persuasion=9, evidence=8, data=7, description="'대기업 저격수' 원조.", ability_name="[대기업 저격]", ability_desc="대기업/외국계 법인세 카드 피해 +25%."),
    "kim": TaxManCard(name="김철주", grade_num=6, position="조사반장", cost=0, hp=130, focus=3, analysis=6, persuasion=8, evidence=9, data=5, description="'지하경제' 양성화 전문가.", ability_name="[압수수색]", ability_desc="'현장 압수수색' 사용 시 15% 확률로 '결정적 증거' 획득."),
    "oh": TaxManCard(name="전필성", grade_num=7, position="조사반장", cost=0, hp=140, focus=3, analysis=7, persuasion=6, evidence=7, data=8, description="TIS 초창기 멤버.", ability_name="[데이터 마이닝]", ability_desc="적출액 70+ 데이터 카드 피해 +15."),
    "jo": TaxManCard(name="조용규", grade_num=7, position="조사반장", cost=0, hp=100, focus=4, analysis=9, persuasion=7, evidence=6, data=7, description="'세금 전도사' 교육원장.", ability_name="[세법 교본]", ability_desc="'판례 제시', '법령 재검토' 효과 2배."),
    "park": TaxManCard(name="박지연", grade_num=8, position="일반조사관", cost=0, hp=90, focus=4, analysis=7, persuasion=5, evidence=6, data=7, description="'세법 신동' 8급 특채.", ability_name="[법리 검토]", ability_desc="턴 첫 분석/설득 카드 비용 -1."),
    "lee": TaxManCard(name="이철수", grade_num=7, position="일반조사관", cost=0, hp=100, focus=3, analysis=5, persuasion=5, evidence=5, data=5, description="열정 넘치는 7급 신입.", ability_name="[기본기]", ability_desc="'기본 비용 분석', '단순 경비 처리' 카드 피해 +8.")
}

# [과세논리 카드 DB] (변경 없음)
LOGIC_CARD_DB = {
    "c_tier_01": LogicCard(name="단순 자료 대사", cost=0, base_damage=5, tax_type=['부가세', '법인세'], attack_category=['공통'], description="매입/매출 자료 단순 비교.", text="자료 대사 기본 습득."),
    "c_tier_02": LogicCard(name="법령 재검토", cost=0, base_damage=0, tax_type=['공통'], attack_category=['공통'], description="카드 1장 뽑기.", text="관련 법령 재검토.", special_effect={"type": "draw", "value": 1}),
    "util_01": LogicCard(name="초과근무", cost=1, base_damage=0, tax_type=['공통'], attack_category=['공통'], description="카드 2장 뽑기.", text="밤샘 근무로 단서 발견!", special_effect={"type": "draw", "value": 2}),
    "basic_01": LogicCard(name="기본 경비 적정성 검토", cost=1, base_damage=10, tax_type=['법인세'], attack_category=['비용'], description="기본 비용 처리 적정성 검토.", text="법인세법 비용 조항 분석."),
    "basic_02": LogicCard(name="단순 경비 처리 오류 지적", cost=1, base_damage=12, tax_type=['법인세'], attack_category=['비용'], description="증빙 미비 경비 지적.", text="증빙 대조 기본 습득."),
    "b_tier_04": LogicCard(name="세금계산서 대사", cost=1, base_damage=15, tax_type=['부가세'], attack_category=['수익', '비용'], description="매입/매출 세금계산서 합계표 대조.", text="합계표 불일치 확인."),
    "c_tier_03": LogicCard(name="가공 증빙 수취 분석", cost=2, base_damage=15, tax_type=['법인세', '부가세'], attack_category=['비용'], description="실물 없는 증빙 수취 분석.", text="가짜 세금계산서 흐름 파악."),
    "corp_01": LogicCard(name="접대비 한도 초과", cost=2, base_damage=25, tax_type=['법인세'], attack_category=['비용'], description="법정 한도 초과 접대비 손금불산입.", text="법인세법 접대비 조항 습득."),
    "b_tier_03": LogicCard(name="판례 제시", cost=2, base_damage=22, tax_type=['공통'], attack_category=['공통'], description="유사 오류 판례 제시.", text="대법원 판례 제시.", special_bonus={'target_method': '단순 오류', 'multiplier': 2.0, 'bonus_desc': '단순 오류에 2배 피해'}),
    "b_tier_05": LogicCard(name="인건비 허위 계상", cost=2, base_damage=30, tax_type=['법인세'], attack_category=['비용'], description="미근무 친인척 인건비 처리.", text="급여대장-근무 내역 불일치 확인."),
    "util_02": LogicCard(name="빅데이터 분석", cost=2, base_damage=0, tax_type=['공통'], attack_category=['공통'], description="적 혐의 유형과 일치하는 카드 1장 서치.", text="TIS 빅데이터 패턴 발견!", special_effect={"type": "search_draw", "value": 1}),
    "corp_02": LogicCard(name="업무 무관 자산 비용 처리", cost=3, base_damage=35, tax_type=['법인세'], attack_category=['비용'], description="대표 개인 차량 유지비 등 적발.", text="벤츠 운행일지 확보!", special_bonus={'target_method': '고의적 누락', 'multiplier': 1.5, 'bonus_desc': '고의적 누락에 1.5배 피해'}),
    "b_tier_01": LogicCard(name="금융거래 분석", cost=3, base_damage=45, tax_type=['법인세'], attack_category=['수익', '자본'], description="의심 자금 흐름 추적.", text="FIU 분석 기법 습득."),
    "b_tier_02": LogicCard(name="현장 압수수색", cost=3, base_damage=25, tax_type=['공통'], attack_category=['공통'], description="현장 방문, 장부-실물 대조.", text="재고 불일치 확인.", special_bonus={'target_method': '고의적 누락', 'multiplier': 2.0, 'bonus_desc': '고의적 누락에 2배 피해'}),
    "a_tier_02": LogicCard(name="차명계좌 추적", cost=3, base_damage=50, tax_type=['법인세', '부가세'], attack_category=['수익'], description="타인 명의 계좌 수입 추적.", text="차명계좌 흐름 파악.", special_bonus={'target_method': '고의적 누락', 'multiplier': 2.0, 'bonus_desc': '고의적 누락에 2배 피해'}),
    "a_tier_01": LogicCard(name="자금출처조사", cost=4, base_damage=90, tax_type=['법인세'], attack_category=['자본'], description="고액 자산가 불분명 자금 출처 추적.", text="수십 개 차명계좌 흐름 파악."),
    "s_tier_01": LogicCard(name="국제거래 과세논리", cost=4, base_damage=65, tax_type=['법인세'], attack_category=['자본'], description="TP 조작, 역외탈세 적발.", text="BEPS 보고서 이해.", special_bonus={'target_method': '자본 거래', 'multiplier': 2.0, 'bonus_desc': '자본 거래에 2배 피해'}),
    "s_tier_02": LogicCard(name="조세피난처 역외탈세", cost=5, base_damage=130, tax_type=['법인세'], attack_category=['자본'], description="SPC 이용 해외 소득 은닉 적발.", text="BVI, 케이맨 SPC 실체 규명.", special_bonus={'target_method': '자본 거래', 'multiplier': 1.5, 'bonus_desc': '자본 거래에 1.5배 피해'}),
}

# [조사도구 DB] (변경 없음)
ARTIFACT_DB = {
    "coffee": Artifact(name="☕ 믹스 커피", description="턴 시작 시 집중력 +1.", effect={"type": "on_turn_start", "value": 1, "subtype": "focus"}),
    "forensic": Artifact(name="💻 포렌식 장비", description="효과 없음 (명확도 제거됨).", effect={"type": "on_battle_start", "value": 0, "subtype": "none"}),
    ...
    "recorder": Artifact(name="🎤 녹음기", description="효과 없음 (명확도 제거됨).", effect={"type": "on_turn_start", "value": 0, "subtype": "none"}),
    "book": Artifact(name="📖 오래된 법전", description="'판례 제시', '법령 재검토' 비용 -1.", effect={"type": "on_cost_calculate", "value": -1, "target_cards": ["판례 제시", "법령 재검토"]})
}

# [기업 DB] (EvasionTactic에서 clarity 값 제거)
COMPANY_DB = [
    Company(
        name="(주)가나푸드", size="소규모", revenue=50, operating_income=5, tax_target=5, team_hp_damage=(5, 10),
        ...
        tactics=[
            # clarity 값 (0.3) 제거됨
            EvasionTactic("사주 개인적 사용", "대표가 배우자 명의 외제차 리스료 월 500만원 법인 처리, 주말 골프 비용, 자녀 학원비 등 법인카드로 결제.", 3, tax_type='법인세', method_type='고의적 누락', tactic_category='비용'),
            # clarity 값 (0.5) 제거됨
            EvasionTactic("증빙 미비 경비", "실제 거래 없이 서류상 거래처 명절 선물 1천만원 꾸미고, 관련 증빙(세금계산서, 입금표) 제시 못함.", 2, tax_type=['법인세', '부가세'], method_type='단순 오류', tactic_category='비용')
        ],
        defense_actions=["담당 세무사가 시간 끌기.", "대표가 '사실무근' 주장.", "경리 직원이 '실수' 변명."]
    ),
    Company(
        name="㈜넥신 (Nexin)", size="중견기업", revenue=1000, operating_income=100, tax_target=20, team_hp_damage=(10, 25),
        description="급성장 게임/IT 기업. 복잡한 지배구조와 관계사 거래.",
        real_case_desc="[교육] IT 기업은 과/면세 구분이 복잡한 점을 악용하는 경우가 있습니다(SW 개발=면세, 유지보수=과세). 특수관계법인(페이퍼컴퍼니)에 용역비를 과다 지급하는 것은 '부당행위계산부인' 대상입니다.",
        tactics=[
            # clarity 값 (0.2) 제거됨
            EvasionTactic("과면세 오류", "과세 대상 'SW 유지보수' 용역 매출 5억원을 면세 'SW 개발'로 위장 신고하여 부가세 누락.", 8, tax_type='부가세', method_type='단순 오류', tactic_category='수익'),
            # clarity 값 (0.1) 제거됨
            EvasionTactic("관계사 부당 지원", "대표 아들 소유 페이퍼컴퍼니에 '경영 자문' 명목으로 시가(월 500)보다 높은 월 3천만원 지급.", 12, tax_type='법인세', method_type='자본 거래', tactic_category='자본')
        ],
        defense_actions=["회계법인이 '정상 거래' 주장.", "자료가 '서버 오류'로 삭제 주장.", "실무자가 '모른다'며 비협조."]
    ),
    Company(
        name="(주)한늠석유 (자료상)", size="중견기업", revenue=500, operating_income=-10, tax_target=30, team_hp_damage=(15, 30),
        ...
        tactics=[
            # clarity 값 (0.3) 제거됨
            EvasionTactic("허위 세금계산서 발행", "실물 없이 폭탄업체로부터 받은 허위 세금계산서(가짜 석유) 수십억 원어치를 최종 소비자에게 발행하여 매입세액 부당 공제.", 20, tax_type='부가세', method_type='고의적 누락', tactic_category='비용'),
            # clarity 값 (0.5) 제거됨
            EvasionTactic("가공 매출 누락", "대포통장 등 차명계좌로 매출 대금 수백억원 수령 후, 세금계산서 미발행으로 부가세/법인세 소득 누락.", 10, tax_type=['법인세', '부가세'], method_type='고의적 누락', tactic_category='수익')
        ],
        defense_actions=["대표 해외 도피.", "사무실 텅 빔 (페이퍼컴퍼니).", "대포폰/대포통장 외 단서 없음."]
    ),
     Company(
        name="㈜삼숭물산 (Samsoong)", size="대기업", revenue=50000, operating_income=2000, tax_target=100, team_hp_damage=(20, 40),
        ...
        tactics=[
            # clarity 값 (0.1) 제거됨
            EvasionTactic("일감 몰아주기", "총수 2세 지분 100% 비상장 'A사'에 그룹 SI 용역을 수의계약으로 고가 발주, 연 수천억원 이익 몰아줌.", 50, tax_type='법인세', method_type='자본 거래', tactic_category='자본'),
            # clarity 값 (0.0) 제거됨
            EvasionTactic("가공 세금계산서 수취", "실거래 없는 유령 광고대행사로부터 수백억 원대 가짜 세금계산서 받아 비용 부풀리고 부가세 부당 환급.", 30, tax_type='부가세', method_type='고의적 누락', tactic_category='비용'),
            # clarity 값 (0.0) 제거됨
            EvasionTactic("불공정 합병", "총수 일가 유리하도록 계열사 합병 비율 조작, 편법으로 경영권 승계 및 이익 증여.", 20, tax_type='법인세', method_type='자본 거래', tactic_category='자본')
        ],
        defense_actions=["최고 로펌 '김&장' 대응팀 꾸림.", "로펌 '정상 경영 활동' 의견서 제출.", "언론에 '과도한 세무조사' 여론전.", "정치권 통해 조사 중단 압력."]
    ),
    Company(
        name="구갈 코리아(유) (Googal)", size="외국계", revenue=2000, operating_income=300, tax_target=80, team_hp_damage=(15, 30),
        ...
        tactics=[
            # clarity 값 (0.1) 제거됨
            EvasionTactic("이전가격(TP) 조작", "버뮤다 페이퍼컴퍼니 자회사에 국내 매출 상당 부분을 'IP 사용료' 명목으로 지급하여 국내 이익 축소.", 50, tax_type='법인세', method_type='자본 거래', tactic_category='자본'),
            # clarity 값 (0.2) 제거됨
            EvasionTactic("고정사업장 미신고", "국내 서버팜 운영하며 광고 수익 창출함에도 '단순 지원 용역'으로 위장, 고정사업장 신고 회피.", 30, tax_type='법인세', method_type='고의적 누락', tactic_category='수익')
        ],
        defense_actions=["미 본사 '영업 비밀' 이유로 자료 제출 거부.", "조세 조약 근거 상호 합의(MAP) 신청 압박.", "자료 영어로만 제출, 번역 지연."]
    ),
    Company(
        name="(주)씨엔해운 (C&)", size="대기업", revenue=10000, operating_income=500, tax_target=150, team_hp_damage=(25, 45),
        ...
        tactics=[
            # clarity 값 (0.1) 제거됨
            EvasionTactic("역외탈세 (SPC)", "파나마, BVI 등 페이퍼컴퍼니(SPC) 명의로 선박 운용, 국내 리스료 수입 수천억원 은닉.", 100, tax_type='법인세', method_type='자본 거래', tactic_category='수익'),
            # clarity 값 (0.2) 제거됨
            EvasionTactic("선박 취득가액 조작", "노후 선박 해외 SPC에 저가 양도 후, SPC가 고가로 제3자 매각, 양도 차익 수백억원 은닉.", 50, tax_type='법인세', method_type='고의적 누락', tactic_category='자본')
        ],
...
# --- 3. 게임 상태 초기화 및 관리 --- (변경 없음, 이전 버전과 동일)
def initialize_game():
...
def execute_attack(card_index, tactic_index):
    ...
    is_tax_match = ('공통' in card.tax_type) or (isinstance(tactic.tax_type, list) and any(tt in card.tax_type for tt in tactic.tax_type)) or (tactic.tax_type in card.tax_type)
    if not is_tax_match:
    ...
    if "이철수" in [m.name for m in st.session_state.player_team] and card.name in ["기본 비용 분석", "단순 경비 처리 오류 지적"]: damage += 8; log_message("✨ [기본기] +8!", "info")
    if "임향수" in [m.name for m in st.session_state.player_team] and ('분석' in card.name or '자료' in card.name or '추적' in card.name): damage += 10; log_message("✨ [기획 조사] +10!", "info")
    ...
    if card.special_bonus and card.special_bonus.get('target_method') == tactic.method_type:
        ...
        if "조용규" in [m.name for m in st.session_state.player_team] and card.name == "판례 제시": bonus_multiplier *= 2; log_message("✨ [세법 교본] '판례 제시' 2배!", "info")
        else: log_message(f"🔥 [정확한 지적!] '{card.name}' perfectly matches '{tactic.method_type}'!", "warning")
    if "한중히" in [m.name for m in st.session_state.player_team] and (company.size == "외국계" or tactic.method_type == '자본 거래'): bonus_multiplier *= 1.3; log_message("✨ [역외탈세 추적] +30%!", "info")
    if "서영택" in [m.name for m in st.session_state.player_team] and (company.size == "대기업" or company.size == "외국계") and '법인세' in card.tax_type: bonus_multiplier *= 1.25; log_message("✨ [대기업 저격] +25%!", "info")
    final_damage = int(damage * bonus_multiplier)
    tactic.exposed_amount += final_damage; company.current_collected_tax += final_damage
    if bonus_multiplier >= 2.0: log_message(f"💥 [치명타!] '{card.name}' hits for **{final_damage}백만원**!", "success")
    elif bonus_multiplier > 1.0: log_message(f"👍 [효과적!] '{card.name}' hits for **{final_damage}백만원**.", "success")
    else: log_message(f"▶️ [적중] '{card.name}' hits for **{final_damage}백만원**.", "success")
    if tactic.exposed_amount >= tactic.total_amount and not getattr(tactic, 'is_cleared', False):
        tactic.is_cleared = True; log_message(f"🔥 [{tactic.name}] fully exposed ({tactic.total_amount}백만원)!", "warning")
...
def enemy_turn():
    ...
    else: log_message(f"{log_prefix} {action_desc} (팀 사기 저하 ❤️-{damage}!)", "error")

def check_battle_end():
    company = st.session_state.current_battle_company
    if company.current_collected_tax >= company.tax_target:
        bonus = company.current_collected_tax - company.tax_target
        log_message(f"🎉 [조사 승리] 목표 {company.tax_target:,}백만원 달성!", "success"); log_message(f"💰 초과 추징 {bonus:,}백만원 획득!", "success")
...
def start_battle(company_template):
    ...
    if st.session_state.player_artifacts: start_artifact = st.session_state.player_artifacts[0]; log_message(f"✨ [조사도구] '{start_artifact.name}' 효과 준비.", "info")
    st.session_state.team_shield = 0; st.session_state.bonus_draw = 0
    for artifact in st.session_state.player_artifacts:
        if artifact.effect["type"] == "on_battle_start":
            if artifact.effect["subtype"] == "shield": shield_gain = artifact.effect["value"]; st.session_state.team_shield += shield_gain; log_message(f"✨ {artifact.name} 보호막 +{shield_gain}!", "info")
            elif artifact.effect["subtype"] == "draw": st.session_state.bonus_draw += artifact.effect["value"]
...
def show_main_menu():
    ...
    st.image("https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?ixlib=rb-4.0.3&q=80&w=1080", caption="국세청 조사국의 풍경 (상상도)", use_container_width=True)
    if st.button("🚨 조사 시작 (신규 게임)", type="primary", use_container_width=True): initialize_game(); st.rerun()
...
def show_map_screen():
    ...
    if st.session_state.current_stage_level < len(company_list):
        ...
        with st.container(border=True):
            ...
            col1, col2 = st.columns(2); col1.metric("매출액", f"{company.revenue:,} 억원"); col2.metric("영업이익", f"{company.operating_income:,} 억원")
            st.warning(f"**예상 턴당 데미지:** {company.team_hp_damage[0]}~{company.team_hp_damage[1]} ❤️"); st.info(f"**목표 추징 세액:** {company.tax_target:,} 백만원 💰")
            with st.expander("Click: 혐의 및 실제 사례 정보"):
                st.info(f"**[교육 정보]**\n{company.real_case_desc}"); st.markdown("---"); st.markdown("**주요 탈루 혐의**")
                for tactic in company.tactics: st.markdown(f"- **{tactic.name}** (`{tactic.tax_type}`, `{tactic.method_type}`, `{tactic.tactic_category}`)")
...
def show_battle_screen():
    ...
    with col_left: # 팀 정보
        ...
        for member in st.session_state.player_team:
            with st.expander(f"**{member.name}** ({member.position}/{member.grade}급)"): st.write(f"HP:{member.hp}/{member.max_hp}, Focus:{member.focus}"); st.info(f"**{member.ability_name}**: {member.ability_desc}"); st.caption(f"({member.description})")
    ...
    with col_mid: # 기업 정보
        st.subheader(f"🏢 {company.name} ({company.size})"); st.progress(min(1.0, company.current_collected_tax/company.tax_target), text=f"💰 목표 세액: {company.current_collected_tax:,}/{company.tax_target:,} (백만원)"); st.markdown("---"); st.subheader("🧾 탈루 혐의 목록")
        ...
        for i, tactic in enumerate(company.tactics):
            ...
            with st.container(border=True):
                st.markdown(f"**{tactic.name}** (`{tactic.tax_type}`/`{tactic.method_type}`/`{tactic.tactic_category}`)"); st.caption(f"_{tactic.description}_")
                if tactic_cleared: st.progress(1.0, text=f"✅ 적발 완료: {tactic.exposed_amount:,}/{tactic.total_amount:,} (백만원)")
                else: st.progress(min(1.0, tactic.exposed_amount/tactic.total_amount), text=f"적발액: {tactic.exposed_amount:,}/{tactic.total_amount:,} (백만원)")
                if is_card_selected and not tactic_cleared:
                    ...
                    is_tax_match = ('공통' in selected_card.tax_type) or (isinstance(tactic.tax_type, list) and any(tt in selected_card.tax_type for tt in tactic.tax_type)) or (tactic.tax_type in selected_card.tax_type)
                    is_category_match = ('공통' in selected_card.attack_category) or (tactic.tactic_category in selected_card.attack_category)
                    button_label, button_type = f"🎯 **{tactic.name}** 공격", "primary"
                    if not is_tax_match: button_label, button_type = f"⚠️ (세목 불일치!) {tactic.name}", "secondary"
                    elif not is_category_match: button_label, button_type = f"⚠️ (유형 불일치!) {tactic.name}", "secondary"
                    if st.button(button_label, key=f"attack_tactic_{i}", use_container_width=True, type=button_type): execute_attack(st.session_state.selected_card_index, i)
        ...
        with tab1:
            ...
            for i, card in enumerate(st.session_state.player_hand):
                ...
                with st.container(border=True):
                    ...
                    st.markdown(card_title); st.caption(f"세목: `{'`. `'.join(card.tax_type)}` | 유형: `{'`. `'.join(card.attack_category)}`"); st.write(card.description); st.write(f"**기본 적출액:** {card.base_damage} 백만원")
                    ...
                    if st.button(button_label, key=f"play_card_{i}", use_container_width=True, disabled=(not can_afford or (is_card_selected and not is_this_card_selected))): select_card_to_play(i)
        with tab2:
...
def show_reward_screen():
    ...
    st.success(f"**{company.name}** 조사 완료. 총 {company.current_collected_tax:,}백만원 추징."); st.markdown("---"); st.subheader("🎁 보상 선택 (카드 1장)")
    ...
    for i, card in enumerate(st.session_state.reward_cards):
        with cols[i]:
            with st.container(border=True):
                st.markdown(f"**{card.name}** | 비용: {card.cost} 🧠"); st.caption(f"세목:`{'`,`'.join(card.tax_type)}`|유형:`{'`,`'.join(card.attack_category)}`"); st.write(card.description); st.info(f"**기본 적출액:** {card.base_damage} 백만원")
                if card.special_bonus: st.warning(f"**보너스:** {card.special_bonus.get('bonus_desc')}")
                if st.button(f"선택: {card.name}", key=f"reward_{i}", use_container_width=True, type="primary"):
                    st.session_state.player_deck.append(card); st.toast(f"[{card.name}] 덱에 추가!", icon="🃏"); del st.session_state.reward_cards
                    st.session_state.game_state = "MAP"; st.session_state.current_stage_level += 1
                    heal_amount = int(st.session_state.team_max_hp * 0.3); st.session_state.team_hp = min(st.session_state.team_max_hp, st.session_state.team_hp + heal_amount); st.toast(f"팀 휴식 (체력 +{heal_amount})", icon="❤️"); st.rerun()

def show_game_over_screen():
    st.header("... 조사 중단 ..."); st.error("팀 체력 소진."); st.metric("최종 총 추징 세액", f"💰 {st.session_state.total_collected_tax:,} 백만원"); st.metric("진행 스테이지", f"📍 {st.session_state.current_stage_level + 1}")
    st.image("https://images.unsplash.com/photo-1554224155-16954a151120?ixlib=rb-4.0.3&q=80&w=1080", caption="지친 조사관들...", use_container_width=True)
...
def show_player_status_sidebar():
    with st.sidebar:
        st.title("👨‍💼 조사팀 현황"); st.metric("💰 총 추징 세액", f"{st.session_state.total_collected_tax:,} 백만원"); st.metric("❤️ 현재 팀 체력", f"{st.session_state.team_hp}/{st.session_state.team_max_hp}")
        if st.session_state.game_state == "BATTLE": st.metric("🛡️ 현재 팀 보호막", f"{st.session_state.get('team_shield', 0)}")
        st.markdown("---"); st.subheader("팀원")
        ...
        st.markdown("---")
        if st.button("게임 포기 (메인 메뉴)", use_container_width=True): st.session_state.game_state = "MAIN_MENU"; st.rerun()
...
def main():
    ...
    if st.session_state.game_state == "MAIN_MENU": show_main_menu()
    elif st.session_state.game_state == "MAP": show_map_screen()
    elif st.session_state.game_state == "BATTLE": show_battle_screen()
    elif st.session_state.game_state == "REWARD": show_reward_screen()
    elif st.session_state.game_state == "GAME_OVER": show_game_over_screen()
    if st.session_state.game_state not in ["MAIN_MENU", "GAME_OVER"]: show_player_status_sidebar()
