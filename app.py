...
class LogicCard(Card):
    ...
    def __init__(self, name, description, cost, base_damage, tax_type: list[TaxType], attack_category: list[AttackCategory], text, special_effect=None, special_bonus=None):
        super().__init__(name, description, max(0, cost + 1));
...
* **부가세**: **제3자**가 부담하는 쿠폰 비용, **마일리지** 결제 부분 등 복잡한 **부가세 과세표준** 산정 오류가 발생하기 쉽습니다.""",
    ...
    Company( # 8등
        ...
        tactics=[EvasionTactic("유가보조금 부정수급 공모", "**화물차주**들과 짜고 **허위 세금계산서**(월 10억원) 발행, 실제 주유 없이 **유가보조금** 총 100억원 편취.", 100, [TaxType.VAT, TaxType.COMMON], MethodType.INTENTIONAL, AttackCategory.REVENUE), EvasionTactic("자료상 행위 (중개)", "실물 거래 없이 **폭탄업체**로부터 **가짜 세금계산서**(50억원)를 매입하여 다른 법인에 수수료 받고 판매.", 50, TaxType.VAT, MethodType.INTENTIONAL, AttackCategory.COST)], defense_actions=["대표 해외 도피 시도.", "사무실 잠적 (페이퍼컴퍼니).", "관련 장부 소각 및 증거 인멸 시도."]
    ),
    Company( # 7등
...
* **진행률 조작**: 아파트/상가 **분양률**을 의도적으로 축소 신고하여, **공사 진행률**에 따른 **수입금액**을 과소 계상합니다.""",
    ...
    Company( # 6등
        ...
        tactics=[EvasionTactic("개발비 과다 자산화", "임상 **실패 가능성**이 높은 **신약 파이프라인** 관련 지출 200억원을 **비용**이 아닌 **무형자산(개발비)**으로 처리하여 **법인세** 이연/탈루.", 180, TaxType.CORP, MethodType.ERROR, AttackCategory.COST), EvasionTactic("IP 저가 양도", "핵심 **신약 특허권**을 **조세피난처** 소재 **페이퍼컴퍼니** 자회사에 **정상 가격**(150억원)보다 현저히 낮은 30억원에 양도.", 120, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL)], defense_actions=["'회계 기준'에 따른 정상적 처리 주장.", "신약 가치 평가는 '미래 불확실성' 반영 필요.", "글로벌 스탠다드라며 자료 제출 비협조."]
    ),
    # --- A Group (Hard, 3-5th) ---
    Company( # 5등
...
* **경영권 분쟁**: **형제간 경영권 다툼** 과정에서 **비자금** 조성 또는 **회계 부정** 발생 가능성.""",
        tactics=[EvasionTactic("계열사 부당 자금 대여", "**총수 일가** 지분이 높은 **(주)로떼정보**에 **업무 무관** 가지급금 500억원을 **무상**으로 대여.", 500, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL), EvasionTactic("부동산 고가 매입", "경영 악화된 **계열사**의 **토지**를 **정상가**(300억)보다 높은 **500억원**에 매입하여 부당 지원.", 300, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL)], defense_actions=["'경영 정상화'를 위한 불가피한 지원 주장.", "자산 평가는 '외부 회계법인' 용역 결과.", "경영권 분쟁 관련 자료 제출 거부."]
    ),
    Company( # 4등
        ...
        tactics=[EvasionTactic("일감 몰아주기 (통행세)", "**총수 자녀 회사**를 거래 중간에 끼워넣어 **통행세** 명목으로 연 500억원 부당 지원.", 500, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL), EvasionTactic("불공정 합병", "**총수 일가**에 유리하게 **계열사 합병 비율**을 산정하여 **이익** 200억원 증여.", 300, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL), EvasionTactic("해외 현지법인 부당 지원", "**싱가포르 자회사**에 **업무 관련성** 없는 **컨설팅 수수료** 명목으로 400억원 부당 지급.", 400, TaxType.CORP, MethodType.INTENTIONAL, AttackCategory.REVENUE)], defense_actions=["대형 로펌 '**태평양**' 자문, '경영상 판단' 주장.", "공정위 등 타 부처 심의 결과 제시하며 반박.", "언론 통해 '**반기업 정서**' 프레임 활용.", "국회 통한 입법 로비 시도."]
    ),
    Company( # 3등
        ...
        tactics=[EvasionTactic("역외탈세 (SPC 소득 은닉)", "**라이베리아** 등 **SPC** 명의 선박 **운항 소득** 1조 2천억원을 국내 미신고 및 해외 은닉.", 1000, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.REVENUE), EvasionTactic("선박 매각 차익 은닉", "**노후 선박**을 해외 SPC에 **저가** 양도 후, SPC가 제3자에 **고가** 매각하는 방식 **양도 차익** 600억원 해외 은닉.", 600, TaxType.CORP, MethodType.INTENTIONAL, AttackCategory.CAPITAL)], defense_actions=["해외 SPC는 '독립된 법인격' 주장.", "국제 해운 관행 및 현지 법률 준수 항변.", "**조세정보교환협정** 미체결국 이용, 자료 확보 방해.", "해운 불황으로 인한 '경영상 어려움' 호소."]
    ),
...
* **디지털 서비스 소득**: 국내 이용자 대상 **광고 수익**, **클라우드 서비스** 제공 대가 등의 **원천지** 규명 및 과세 문제.""",
        tactics=[EvasionTactic("이전가격(TP) 조작 - 경영지원료", "**싱가포르 지역본부**에 **실제 역할** 대비 과도한 **경영지원 수수료** 600억원 지급, 국내 이익 축소.", 600, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL), EvasionTactic("고정사업장 회피", "국내 **클라우드 서버** 운영 및 **기술 지원** 인력이 **핵심적 역할** 수행함에도 **고정사업장** 미신고, 관련 소득 400억원 과세 회피.", 400, TaxType.CORP, MethodType.INTENTIONAL, AttackCategory.REVENUE)], defense_actions=["미국 본사 '**기술 이전 계약**' 근거 정상 거래 주장.", "**조세 조약** 및 **OECD 가이드라인** 해석 다툼 예고.", "**상호합의절차(MAP)** 신청 통한 시간 끌기 전략.", "각국 과세 당국 간 **정보 부족** 악용."]
    ),
    Company( # 1등
...
* **부가세**: **클라우드 서비스**가 '**국외 공급 용역**'인지 '**국내 공급 용역**'인지에 따라 **부가세** 과세 여부 달라짐.""",
        tactics=[EvasionTactic("고정사업장(PE) 회피", "국내 **대규모 데이터센터(IDC)**가 **클라우드 서비스**의 **핵심 사업** 수행함에도 **'예비적/보조적' 활동**이라 주장하며 관련 **법인세** 1조원 신고 누락.", 1000, TaxType.CORP, MethodType.INTENTIONAL, AttackCategory.REVENUE), EvasionTactic("이전가격 조작 - 로열티", "국내 **E-커머스** 사업 수익 대부분을 **'브랜드 사용료'** 및 **'플랫폼 로열티'** 명목으로 **룩셈부르크** 본사에 과다 지급.", 800, TaxType.CORP, MethodType.CAPITAL_TX, AttackCategory.CAPITAL)], defense_actions=["**조세 조약** 상 고정사업장 정의에 미부합 주장.", "클라우드 서버는 '단순 저장 장치'라고 항변.", "미국 **IRS**와의 **이중과세** 문제 제기 (MAP).", "한국 정부 '디지털세' 도입 반대 로비."]
    ),
]
...
def develop_tax_logic(): # 과세 논리 개발
    ...
    if remaining_tactics:
        for t in remaining_tactics:
            ...
            if isinstance(t.tax_type, list):
                 for tt in t.tax_type:
                    if hasattr(tt, 'value'): target_cats.add(tt.value)
            elif hasattr(t.tax_type, 'value'):
                 target_cats.add(t.tax_type.value)

            if hasattr(t.method_type, 'value'):
    ...
    for card in LOGIC_CARD_DB.values():
        ...
        card_cats_values = [c.value for c in card.attack_category]
...
def check_battle_end():
    ...
    if company.current_collected_tax >= company.tax_target:
        ...
        if next_state != "REWARD_BONUS" and random.random() < 0.40:
...
def start_battle(co_template):
    ...
    for t in co.tactics:
        tax_str = "타입 오류"
        if isinstance(t.tax_type, list):
            valid_types = [tx.value for tx in t.tax_type if hasattr(tx, 'value')]
            if valid_types: tax_str = '/'.join(valid_types)
        elif hasattr(t.tax_type, 'value'):
            tax_str = t.tax_type.value
...
def go_to_next_stage(add_card=None):
    ...
    # 보너스 보상 임시 저장소 클리어
    if 'bonus_reward_artifact' in st.session_state: st.session_state.bonus_reward_artifact = None
    if 'bonus_reward_member' in st.session_state: st.session_state.bonus_reward_member = None
    ...
    if stage < len(companies):
        ...
        with st.container(border=True):
            ...
            with st.expander("🔍 혐의 및 실제 사례 정보 보기"):
                ...
                st.markdown(co.real_case_desc) # 복원된 교육 정보 표시
                ...
                if not co.tactics:
                    ...
                    for i, t in enumerate(co.tactics):
                        try:
                            ...
                            if isinstance(t.tax_type, list):
                                valid_types = [tx.value for tx in t.tax_type if hasattr(tx, 'value')]
                                if valid_types: t_types_str = '/'.join(valid_types)
                            elif hasattr(t.tax_type, 'value'):
                                t_types_str = t.tax_type.value
                            method_val = t.method_type.value if hasattr(t.method_type, 'value') else "메소드 오류"
                            category_val = t.tactic_category.value if hasattr(t.tactic_category, 'value') else "카테고리 오류"
                            # --- 수정 끝 ---
                    ...
                    with st.container(border=True):
                        ...
                        if is_sel and not cleared:
                            if st.session_state.selected_card_index < len(st.session_state.player_hand):
                                ...
                                # is_tax, is_cat 계산 시 card의 속성(list)과 tactic의 속성(list or object) 비교
                                card_tax_values = [ct.value for ct in card.tax_type if hasattr(ct, 'value')]
                                tactic_tax_values = []
                                if isinstance(t.tax_type, list):
                                    tactic_tax_values = [tt.value for tt in t.tax_type if hasattr(tt, 'value')]
                                elif hasattr(t.tax_type, 'value'):
                                    tactic_tax_values = [t.tax_type.value]

                                card_cat_values = [cc.value for cc in card.attack_category if hasattr(cc, 'value')]
                                tactic_cat_value = t.tactic_category.value if hasattr(t.tactic_category, 'value') else None
                                ...
                                disabled = False
                                if not is_tax: label, type, help, disabled = f"⚠️ (세목 불일치!)", "secondary", f"세목 불일치! '{', '.join(card_tax_values)}' 카드는 '{', '.join(tactic_tax_values)}' 혐의에 사용 불가.", True
                                elif not is_cat: label, type, help, disabled = f"⚠️ (유형 불일치!)", "secondary", f"유형 불일치! '{', '.join(card_cat_values)}' 카드는 '{tactic_cat_value}' 혐의에 사용 불가.", True
        ...
        with hand_container:
            ...
            for i, card in enumerate(st.session_state.player_hand):
                ...
                with st.container(border=True):
                    ...
                    disabled = not afford
                    c_types_values = [t.value for t in card.tax_type if hasattr(t, 'value')]
                    c_cats_values = [c.value for c in card.attack_category if hasattr(c, 'value')]
...
def show_reward_screen():
    ...
    for i, card in enumerate(st.session_state.reward_cards):
        with cols[i]:
            with st.container(border=True):
                types_values = [t.value for t in card.tax_type if hasattr(t, 'value')]
                cats_values = [c.value for c in card.attack_category if hasattr(c, 'value')]
                st.markdown(f"**{card.name}**|비용:{card.cost}🧠"); # 기본 비용 표시
                st.caption(f"세목:`{'`,`'.join(types_values)}`|유형:`{'`,`'.join(cats_values)}`"); st.markdown(card.description);
...
def main():
    ...
    pages = {
        ...
        "REWARD_BONUS": show_reward_bonus_screen, # 상태와 함수 연결
