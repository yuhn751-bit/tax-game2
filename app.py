알겠습니다. 팀원 중복 오류 수정, 교육 정보 강화, 캐릭터 정보 수정 및 추가, 조사 도구 추가, 그리고 특정 기업 조사 시 관련 카드 추가 기능까지 모두 반영하여 코드를 업데이트했습니다.

**주요 변경 사항:**

1.  **팀원 중복 오류 수정:** `initialize_game` 함수에서 리더를 제외한 팀원을 뽑을 때, 리더 자신과 중복되지 않도록 수정했습니다.
2.  **교육 정보 강화:** `COMPANY_DB`의 `real_case_desc`에 각 기업 유형별 조사 시 **주요 착안 사항, 관련 법규 키워드, 증빙 자료 확보 방안** 등 실질적인 내용을 대폭 보강했습니다.
3.  **캐릭터 정보 수정:** `TAX_MAN_DB` 설명에서 "[현직]", "[가상]" 등의 상태 정보를 모두 제거했습니다.
4.  **리더 드래프트 확장:** 게임 시작 시 리더 후보를 **4명** 제시하도록 수정했습니다.
5.  **신규 캐릭터 2명 추가:** **강민수(現 서울청장), 김창기(現 국세청장)** 캐릭터 2명을 `TAX_MAN_DB`에 추가하고 능력치를 부여했습니다.
6.  **신규 조사도구 2개 추가:** **'압수물 분석 보고서', '외부 전문가 자문'** 아티팩트 2개를 `ARTIFACT_DB`에 추가했습니다.
7.  **기업 특화 카드 추가 로직:** `start_battle` 함수 수정:
      * 조사 대상 기업의 혐의 목록(`company.tactics`)에 **'자본 거래(Capital Tx)'** 유형이 포함되어 있을 경우, 전투 시작 시 플레이어 덱에 **'자금출처조사' 카드 1장을 임시로 추가**하도록 구현했습니다.
      * 이 카드는 해당 전투 동안에만 덱에 포함됩니다. (다음 전투 시작 시 기본 덱 + 보상 카드로 재구성됨)
      * 관련 로그 메시지도 추가했습니다.
